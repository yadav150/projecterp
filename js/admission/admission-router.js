...
import {
  validateAdmission, validateStep, generateQRToken, getStatusLabel, getStatusBadge,
      ...
      columns: [
        ...
        { key: 'createdAt', label: 'Applied', sortable: true, render: r => fmtDate(r.createdAt) },
        {
          ...
          render: r => rowActions([
            ...
            { icon: ICON.print, label: 'Print', onClick: () => admissionPrint(r.id) },
            {
...
// ---------- New Application – 6 Steps with Validation ----------
function renderNewForm(container) {
  container.innerHTML = "";
  const steps = [
    { id: 'personal', label: 'Personal' },
    { id: 'education', label: 'Education' },
    { id: 'upload', label: 'Upload & Contact' },
  ...
  function validateCurrentStep() {
    const result = validateStep(currentStep, formData, formData.documents || []);
    stepErrors = result.errors;
    return result.valid;
  }

  // ----- Render step with validation feedback -----
  function renderStep(idx) {
    ...
    const errorSummary = el("div", { style: "margin-bottom:12px;" });
    stepContainer.appendChild(errorSummary);
    ...
    if (idx < steps.length - 1) {
      ...
      nextBtn.onclick = () => {
        ...
        if (!validateCurrentStep()) {
          ...
          const errDiv = el("div", {
            class: "card",
            style: "padding:12px;border-color:var(--danger);background:var(--danger-soft);"
          });
          errDiv.appendChild(el("div", { style: "font-weight:600;color:var(--danger);margin-bottom:6px;", text: "Please fix the following errors:" }));
          stepErrors.forEach(err => {
            errDiv.appendChild(el("div", { style: "font-size:13px;color:var(--danger);padding:2px 0;", text: `• ${err}` }));
          });
          errorSummary.appendChild(errDiv);

          // Highlight invalid fields
          highlightInvalidFields(content);
          return;
        }
    ...
    // Auto-validate on render and show errors if any
    validateCurrentStep();
    if (stepErrors.length > 0) {
      ...
      errDiv.appendChild(el("div", { style: "font-weight:600;color:var(--danger);margin-bottom:6px;", text: "Please fill in all required fields:" }));
      stepErrors.forEach(err => {
        errDiv.appendChild(el("div", { style: "font-size:13px;color:var(--danger);padding:2px 0;", text: `• ${err}` }));
      });
      errorSummary.appendChild(errDiv);
  ...
  function highlightInvalidFields(container) {
    ...
    container.querySelectorAll('.input-error, .select-error').forEach(el => {
      el.classList.remove('input-error', 'select-error');
    });

    // Simple highlighting based on errors
    ...
    inputs.forEach(input => {
      ...
      if (!key) return;
      const fieldLabel = input.closest('.form-row')?.querySelector('label')?.textContent || key;
      const hasError = stepErrors.some(err => err.toLowerCase().includes(key.toLowerCase()) || err.includes(fieldLabel));
      if (hasError) {
        input.classList.add('input-error');
        const parent = input.closest('.form-row');
        if (parent) {
          const errorEl = el("div", { class: "field-error", style: "color:var(--danger);font-size:12px;margin-top:4px;", text: "This field is required" });
  ...
  function renderUploadStep(container) {
    ...
    DOC_TYPES.forEach(doc => {
      ...
      if (existing) {
        statusDisplay.textContent = `Uploaded: ${existing.name}`;
      }
      fileInput.onchange = async (e) => {
        ...
        try {
          ...
          statusDisplay.textContent = `Uploaded: ${file.name}`;
  ...
  function renderFinalStep(container) {
    ...
    summary.appendChild(el("div", { style: "font-weight:600;margin-top:12px;padding:4px 0;", text: "Documents:" }));
    DOC_TYPES.forEach(doc => {
      const found = docs.find(d => d.key === doc.key);
      summary.appendChild(el("div", { style: "display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);" }, [
        el("span", { text: doc.label }),
        el("span", { text: found ? '✅ Uploaded' : '❌ Missing', style: `color:${found ? 'var(--success)' : 'var(--danger)'};` })
      ]));
    });
    ...
    // Validation errors
    const result = validateAdmission(formData, formData.documents || []);
    if (!result.valid) {
      const errDiv = el("div", { class: "card", style: "padding:12px;margin-top:12px;border-color:var(--danger);background:var(--danger-soft);" });
      errDiv.appendChild(el("div", { style: "font-weight:600;color:var(--danger);", text: "Validation errors:" }));
      result.errors.forEach(e => errDiv.appendChild(el("div", { style: "font-size:13px;color:var(--danger);padding:2px 0;", text: `• ${e}` })));
  ...
  async function submitAdmission() {
    const result = validateAdmission(formData, formData.documents || []);
    if (!result.valid) {
      toast({ type: 'error', title: 'Validation failed', message: result.errors.join('; ') });
      return;
    }
    if (!admissionId) {
