...
var formData = {};
var formStep = 1;
var draftId = null; // Firebase key of the draft being edited
    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        try {
          ...
          resolve({ url: response.secure_url, publicId: response.public_id, format: response.format, bytes: response.bytes });
        } catch (e) { reject(new Error('Invalid response from Cloudinary')); }
      } else {
        reject(new Error('Upload failed: ' + xhr.status + ' ' + xhr.statusText));
      }
    };
    xhr.onerror = function() { reject(new Error('Network error uploading to Cloudinary')); };
    xhr.send(formData);
  });
}

// ============================================================
// MAIN RENDER – decides list or form based on hash
// ============================================================
function render(container) {
  ...
  if (hash.includes('/new')) {
    ...
    if (id) renderForm(container, 'edit', id);
...
// ============================================================
// RENDER LIST (Admission List with Drafts)
// ============================================================
...
function loadAdmissions() {
  db.ref(ADMISSIONS_PATH).once('value').then(function(snapshot) {
    ...
    allAdmissions.sort(function(a, b) { return (b.updatedAt || 0) - (a.updatedAt || 0); });
...
function viewAdmission(id) {
  ...
  if (!item) { window.showToast('Record not found.', 'error'); return; }
  var s = item.student || {};
  var p = item.parent || {};
  var a = item.address || {};
  ...
  var bodyHtml = '' +
    ...
    '</div>' +
    '<div class="two-col"><div><h4 style="margin:0 0 8px;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;">Student Information</h4>' +
    ...
    '</div><div><h4 style="margin:0 0 8px;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;">Parent / Guardian</h4>' +
    ...
    '</div></div>' +
    '<div class="two-col" style="margin-top:12px;"><div><h4 style="margin:0 0 8px;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;">Address</h4>' +
    ...
    '</div><div><h4 style="margin:0 0 8px;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;">Previous Academic</h4>' +
...
// ============================================================
// RENDER FORM (Full Page Multi‑Step with Auto‑Save Draft)
// ============================================================
  ...
  if (currentMode === 'edit' && currentEditId) {
    db.ref(ADMISSIONS_PATH + '/' + currentEditId).once('value').then(function(snapshot) {
      if (snapshot.exists()) {
        ...
        draftId = currentEditId;
        // If the record is already submitted, we still allow editing only if not submitted (but we'll allow if draft)
        if (formData.status === 'Submitted') {
          window.showToast('This admission is already submitted. You cannot edit it.', 'error');
    ...
    // New admission – check if there's an existing draft for this session (optional)
    // We'll simply create a new draft on first save.
    formData = {};
    draftId = null;
    renderFormPage(container);
  }
...
function renderFormPage(container) {
  var title = currentMode === 'edit' ? 'Edit Admission' : 'New Admission';
  var html = '' +
    '<div class="page-header">' +
      '<div><h1 class="page-title">' + title + '</h1><p class="page-subtitle">Multi-step admission workflow</p></div>' +
      '<div class="page-actions">' +
        '<button class="btn btn-outline" onclick="window.admissionModule.cancelForm()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg> Back to List</button>' +
      '</div>' +
    '</div>' +
    '<div class="card">' +
  ...
  // Initial render
  renderStepContent();
  // Auto-populate previous class if class is set
  autoPopulatePreviousClass();
}
...
function renderStep(step) {
  ...
  if (step > 1) {
    ...
    html += '<button class="btn btn-outline" onclick="window.admissionModule.cancelForm()">Cancel</button>';
  }
  if (step < totalSteps) {
...
function renderStepContent() {
  ...
  var data = formData;
  switch(step) {
    case 1: body.innerHTML = renderStep1(data); break;
    case 2: body.innerHTML = renderStep2(data); break;
    case 3: body.innerHTML = renderStep3(data); break;
    case 4: body.innerHTML = renderStep4(data); break;
    case 5: body.innerHTML = renderStep5(data); break;
    case 6: body.innerHTML = renderStep6(data); break;
    case 7: body.innerHTML = renderStep7(data); break;
  }
  // Re-bind event listeners for auto-populate
  if (step === 4) autoPopulatePreviousClass();
}
...
function renderStep1(data) {
  ...
  return '' +
    '<div class="form-grid">' +
      ...
      '<div class="form-row"><label>Student Name <span class="req">*</span></label><input class="input" id="s1-name" value="' + (s.name || '') + '" placeholder="Enter full name" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Date of Birth <span class="req">*</span></label><input class="input" type="date" id="s1-dob" value="' + (s.dob || '') + '" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Gender <span class="req">*</span></label><select class="select" id="s1-gender" onchange="window.admissionModule.saveDraft()"><option value="">Select</option><option value="Male"' + (s.gender === 'Male' ? ' selected' : '') + '>Male</option><option value="Female"' + (s.gender === 'Female' ? ' selected' : '') + '>Female</option><option value="Other"' + (s.gender === 'Other' ? ' selected' : '') + '>Other</option></select></div>' +
      '<div class="form-row"><label>Aadhaar Number</label><input class="input" id="s1-aadhaar" value="' + (s.aadhaar || '') + '" placeholder="XXXX-XXXX-XXXX" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Class <span class="req">*</span></label><select class="select" id="s1-class" onchange="window.admissionModule.saveDraft(); window.admissionModule.autoPopulatePreviousClass();"><option value="">Select</option><option value="Nursery"' + (s.class === 'Nursery' ? ' selected' : '') + '>Nursery</option><option value="KG1"' + (s.class === 'KG1' ? ' selected' : '') + '>KG1</option><option value="KG2"' + (s.class === 'KG2' ? ' selected' : '') + '>KG2</option><option value="1"' + (s.class === '1' ? ' selected' : '') + '>Class 1</option><option value="2"' + (s.class === '2' ? ' selected' : '') + '>Class 2</option><option value="3"' + (s.class === '3' ? ' selected' : '') + '>Class 3</option><option value="4"' + (s.class === '4' ? ' selected' : '') + '>Class 4</option><option value="5"' + (s.class === '5' ? ' selected' : '') + '>Class 5</option><option value="6"' + (s.class === '6' ? ' selected' : '') + '>Class 6</option><option value="7"' + (s.class === '7' ? ' selected' : '') + '>Class 7</option><option value="8"' + (s.class === '8' ? ' selected' : '') + '>Class 8</option><option value="9"' + (s.class === '9' ? ' selected' : '') + '>Class 9</option><option value="10"' + (s.class === '10' ? ' selected' : '') + '>Class 10</option></select></div>' +
      '<div class="form-row"><label>Section</label><select class="select" id="s1-section" onchange="window.admissionModule.saveDraft()"><option value="NA"' + (!s.section || s.section === 'NA' ? ' selected' : '') + '>NA</option><option value="A"' + (s.section === 'A' ? ' selected' : '') + '>A</option><option value="B"' + (s.section === 'B' ? ' selected' : '') + '>B</option><option value="C"' + (s.section === 'C' ? ' selected' : '') + '>C</option></select></div>' +
    '</div>';
}
...
function renderStep2(data) {
  ...
  return '' +
    '<div class="form-grid">' +
      '<div class="form-row"><label>Father\'s Name <span class="req">*</span></label><input class="input" id="s2-father" value="' + (p.fatherName || '') + '" placeholder="Enter father\'s name" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Mother\'s Name <span class="req">*</span></label><input class="input" id="s2-mother" value="' + (p.motherName || '') + '" placeholder="Enter mother\'s name" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Guardian Name</label><input class="input" id="s2-guardian-name" value="' + (p.guardianName || '') + '" placeholder="If applicable" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Guardian Relation</label><input class="input" id="s2-guardian-relation" value="' + (p.guardianRelation || '') + '" placeholder="e.g. Uncle, Grandparent" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Contact Number <span class="req">*</span></label><input class="input" id="s2-contact" value="' + (p.contact || '') + '" placeholder="10-digit mobile number" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Email Address</label><input class="input" type="email" id="s2-email" value="' + (p.email || '') + '" placeholder="email@example.com" onchange="window.admissionModule.saveDraft()" /></div>' +
    '</div>';
}
...
function renderStep3(data) {
  ...
  return '' +
    '<div class="form-grid">' +
      '<div class="form-row" style="grid-column:1/-1;"><label>Present Address <span class="req">*</span></label><textarea class="textarea" id="s3-present" rows="2" onchange="window.admissionModule.saveDraft()">' + (a.present || '') + '</textarea></div>' +
      '<div class="form-row" style="grid-column:1/-1;"><label>Permanent Address</label><textarea class="textarea" id="s3-permanent" rows="2" onchange="window.admissionModule.saveDraft()">' + (a.permanent || '') + '</textarea></div>' +
      '<div class="form-row"><label>Village / Town <span class="req">*</span></label><input class="input" id="s3-village" value="' + (a.village || '') + '" placeholder="Village or town name" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>District <span class="req">*</span></label><input class="input" id="s3-district" value="' + (a.district || '') + '" placeholder="District name" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>State <span class="req">*</span></label><input class="input" id="s3-state" value="' + (a.state || '') + '" placeholder="State name" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>PIN Code <span class="req">*</span></label><input class="input" id="s3-pin" value="' + (a.pinCode || '') + '" placeholder="6-digit PIN" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Emergency Contact <span class="req">*</span></label><input class="input" id="s3-emergency" value="' + (a.emergencyContact || '') + '" placeholder="10-digit number" onchange="window.admissionModule.saveDraft()" /></div>' +
    '</div>';
}

function renderStep4(data) {
  var p = data.previous || {};
  var cls = (data.student && data.student.class) || '';
  // Auto-populate previous class if not set
  var prevClass = p.class || '';
  return '' +
    '<div class="form-grid">' +
      '<div class="form-row"><label>Previous School</label><input class="input" id="s4-school" value="' + (p.school || '') + '" placeholder="School name" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Previous Class</label><input class="input" id="s4-class" value="' + prevClass + '" placeholder="e.g. 9" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Board</label><input class="input" id="s4-board" value="' + (p.board || '') + '" placeholder="e.g. CBSE, ICSE" onchange="window.admissionModule.saveDraft()" /></div>' +
      '<div class="form-row"><label>Previous Roll Number</label><input class="input" id="s4-roll" value="' + (p.rollNumber || '') + '" placeholder="Previous roll number" onchange="window.admissionModule.saveDraft()" /></div>' +
    '</div>';
}
...
// ---- Auto-populate Previous Class ----
function autoPopulatePreviousClass() {
  var classSelect = document.getElementById('s1-class');
  var prevClassInput = document.getElementById('s4-class');
  if (!classSelect || !prevClassInput) return;
  ...
  var prev = classMap[cls] || '';
  // Only auto-fill if input is empty or if the current value matches the previous auto-filled value
  var currentVal = prevClassInput.value.trim();
  // If current value is empty OR it matches the previous auto-populated value, update it.
  // We'll track if it was auto-populated by a data attribute.
  if (!currentVal || prevClassInput.dataset.auto === 'true') {
    prevClassInput.value = prev;
    prevClassInput.dataset.auto = 'true';
    // Save draft after auto-populate
    saveDraft();
  }
}
...
function goToStep(step) {
  ...
  formStep = step;
  renderStepContent();
  var container = document.getElementById('admission-form-body');
  if (container) {
  ...
  // Auto-populate previous class if on step 4
  if (step === 4) autoPopulatePreviousClass();
...
function saveDraft() {
  ...
  if (draftId) {
    ...
    db.ref(ADMISSIONS_PATH + '/' + draftId).update(data)
      .then(function() {
        // Silently saved
      })
      .catch(function(error) {
    ...
    // Create new draft
    var newRef = db.ref(ADMISSIONS_PATH).push();
    draftId = newRef.key;
    data.id = draftId;
    newRef.set(data)
      .then(function() {
...
function submitAdmission() {
  ...
  db.ref(ENROLLMENT_COUNTERS_PATH + '/' + year).transaction(function(current) {
    ...
    db.ref(COUNTERS_PATH + '/' + rollCounterKey).transaction(function(current) {
      ...
      if (draftId && draftId !== admissionId) {
        updates[ADMISSIONS_PATH + '/' + draftId] = null;
      }
