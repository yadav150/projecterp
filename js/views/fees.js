...
export function FeesView() {
  ...
  page.appendChild(el("div", { class: "page-header" }, [
    ...
    el("div", { class: "page-actions" }, [
      el("button", {
        ...
        html: `${ICON.plus}<span>Collect Fee</span>`
      })
    ])
  ]));
  ...
  [classSel, statusSel, dateFrom, dateTo].forEach(x => x.addEventListener("change", refreshTable));
  ...
  // --- Subscribe to students (needed for dropdown) ---
  if (unsubS) unsubS();
  unsubS = subscribeStudents(v => { students = v || []; });

  // --- Subscribe to fees (real-time updates) ---
  if (unsubF) unsubF();
...
function openCollectFee() {
  ...
  students.forEach(s => {
    studentSel.appendChild(el("option", {
      value: s.id,
      text: `${s.name} · Adm #${s.admissionNumber || "—"} · ${s.class || ""}${s.section ? " " + s.section : ""}`
    }));
  });
